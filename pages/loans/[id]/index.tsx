...
const LoanPage = ({
	...
	const deleteNote = async (noteId, docName) => {
		setLoading(true);
		try {
			// delete doc from aws s3
			if (docName) {
				const deleteUrl = new URL('/api/docs/delete-doc', window.location.href);
				deleteUrl.searchParams.set('docName', docName);

				const res = await fetch(deleteUrl.toString(), { method: "DELETE" }).then((res) => res.json());
			}
